...
- **FFmpeg 合并增强**：新增流信息日志与包级容错，自动跳过损坏包并继续合并
- **异常分片自动重试**：合并失败后生成过滤 m3u8，自动跳过缺失或空分片并重试
- **头尾广告分片裁剪**：针对加密流新增头尾分片裁剪重试策略
- **合并结果校验与状态修复**：恢复历史任务时校验输出文件，缺失时标记错误并触发重试
- **新增 orange-ffmpeg 依赖**：合并流程默认使用精简 FFmpeg 实现 m3u8 加密视频转 mp4
- **FFmpeg 构建优化**：构建脚本加入 crypto/tls 协议支持与 Clash 代理可用性检测
- **合并后清理缓存**：FFmpeg 合并成功后自动清理 ts 缓存文件
